...
{
    ImGuiWindow*        Window;         // AddFrm,Move    // Best candidate window (result->ItemWindow->RootWindowForNav == request->Window)
    ImGuiID             ID;             // AddFrm,Move    // Best candidate item ID
    ImGuiID             FocusScopeId;   // AddFrm,Move    // Best candidate focus scope ID
    ImRect              RectRel;        // AddFrm,Move    // Best candidate bounding box in window relative space
...
struct ImGuiContext
{
    ...
    // Navigation: AddFrm & Move Requests
    bool                    NavAnyRequest;                      // ~~ NavMoveRequest || NavInitRequest this is to perform early out in ItemAdd()
    bool                    NavInitRequest;                     // AddFrm request for appearing window to select first item
    bool                    NavInitRequestFromMove;
    ImGuiNavItemData        NavInitResult;                      // AddFrm request result (first item of the window, or one for which SetItemDefaultFocus() was called)
